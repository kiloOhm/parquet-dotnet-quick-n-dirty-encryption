...
namespace Parquet.Test.Bloom {
    ...
    /// </summary>
    public sealed class Bloom_EndToEnd_Tests {
        ...
        private static ParquetOptions Opts(bool enableBlooms) =>
            new ParquetOptions { EnableBloomFilters = enableBlooms };
        ...
        public async Task Write_With_Bloom_Then_Read_And_Probe() {
            ...
            var writer = new DataColumnWriter(
                ...
                options: new ParquetOptions { EnableBloomFilters = true },
            ...
            var reader = new DataColumnReader(field, ms, chunk, stats, footer, Opts(true));
        ...
        public async Task Write_Without_Bloom_No_Metadata_No_Prune() {
            ...
            var writer = new DataColumnWriter(
                ms, footer, se,
                compressionMethod: CompressionMethod.None,
                options: Opts(enableBlooms: false),
                compressionLevel: System.IO.Compression.CompressionLevel.NoCompression,
                keyValueMetadata: null);
            ...
            var reader = new DataColumnReader(field, ms, chunk, stats, footer, Opts(false));
        ...
        public async Task WriteRead_WithBloom_MultipleRowGroups() {
            ...
            using(ParquetWriter writer = await ParquetWriter.CreateAsync(
                ...
                ms,
                new ParquetOptions {
                    EnableBloomFilters = true // single global knob you added
                })) {
            ...
            using(ParquetReader reader = await ParquetReader.CreateAsync(
                ms,
                new ParquetOptions {
                    EnableBloomFilters = true // readers may optionally use blooms for pruning
                })) {
